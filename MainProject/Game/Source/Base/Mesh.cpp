...
void Mesh::Draw(Camera* pCamera, ShaderProgram* pShader, vec3 pos, fw::Texture* pTexture)
{
    ...
    if(uWorldMat != -1 )
    {
        mat4 mat;
        mat.SetIdentity();
        mat.CreateSRT(1, vec3(pos.x,pos.y,pos.z), vec3(pos.x, pos.y, pos.z));
    ...
    GLint uViewMatrix = glGetUniformLocation( pShader->GetProgram(), "u_ViewMatrix" );
    if(uViewMatrix != -1 )
    {
    ...
    GLint uProjectionMatrix = glGetUniformLocation( pShader->GetProgram(), "u_ProjMatrix" );
    if(uProjectionMatrix != -1 )
    {

		mat4 mat = pCamera->GetProjectionMatrix();

		glUniformMatrix4fv(uProjectionMatrix, 1, false, &mat.m11);
    }
